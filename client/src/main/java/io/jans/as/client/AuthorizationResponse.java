...
public class AuthorizationResponse extends BaseResponse {
    ...
    private static final String DEF_ERROR = "error";
    private static final String DEF_ERROR_DESCRIPTION = "error_description";
    private static final String DEF_ERROR_URI = "error_uri";
    private static final String DEF_STATE = "state";
    private static final String DEF_REDIRECT = "redirect";
        ...
        if (StringUtils.isNotBlank(entity)) {
            try {
                ...
                if (jsonObj.has(DEF_ERROR)) {
                    errorType = AuthorizeErrorResponseType.fromString(jsonObj.getString(DEF_ERROR));
                }
                if (jsonObj.has(DEF_ERROR_DESCRIPTION)) {
                    errorDescription = jsonObj.getString(DEF_ERROR_DESCRIPTION);
                }
                if (jsonObj.has(DEF_ERROR_URI)) {
                    errorUri = jsonObj.getString(DEF_ERROR_URI);
                }
                if (jsonObj.has(DEF_STATE)) {
                    state = jsonObj.getString(DEF_STATE);
                }
                if (jsonObj.has(DEF_REDIRECT)) {
                    location = jsonObj.getString(DEF_REDIRECT);
                }
    ...
    private void loadparams(Map<String, String> params) throws UnsupportedEncodingException {
        ...
        // Error
        if (params.containsKey(DEF_ERROR)) {
            errorType = AuthorizeErrorResponseType.fromString(params.get(DEF_ERROR));
            params.remove(DEF_ERROR);
        }
        if (params.containsKey(DEF_ERROR_DESCRIPTION)) {
            errorDescription = URLDecoder.decode(params.get(DEF_ERROR_DESCRIPTION), StandardCharsets.UTF_8);
            params.remove(DEF_ERROR_DESCRIPTION);
        }
        if (params.containsKey(DEF_ERROR_URI)) {
            errorUri = URLDecoder.decode(params.get(DEF_ERROR_URI), StandardCharsets.UTF_8);
            params.remove(DEF_ERROR_URI);
        }

        loadParamsProc(params);
    }

    private void loadParamsProc(Map<String, String> params) {
